...
def get_basic_features(dp):
    rank_diff = dp.p0_rank - dp.p1_rank
    ...
    rank_points_diff = dp.p0_rank_points - dp.p1_rank_points
    ...
    height_diff = dp.p0_height - dp.p1_height
    age_diff = dp.p0_age - dp.p1_age
    return [rank_diff, rank_logdiff, rank_points_diff, rank_points_logdiff, height_diff, age_diff]


def get_hist_features(dp):
    wins_diff_y = dp.p0_wins_year - dp.p1_wins_year
    played_diff_y = dp.p0_played_year - dp.p1_played_year
    winperc_diff_y = dp.p0_winperc_year - dp.p1_winperc_year

    wins_diff_m = dp.p0_wins_month - dp.p1_wins_month
    played_diff_m = dp.p0_played_month - dp.p1_played_month
    winperc_diff_m = dp.p0_winperc_month - dp.p1_winperc_month

    wins_diff_surf = dp.p0_wins_surface - dp.p1_wins_surface
    played_diff_surf = dp.p0_played_surface - dp.p1_played_surface
    winperc_diff_surf = dp.p0_winperc_surface - dp.p1_winperc_surface

    wins_diff_lev = dp.p0_wins_level - dp.p1_wins_level
    played_diff_lev = dp.p0_played_level - dp.p1_played_level
    winperc_diff_lev = dp.p0_winperc_level - dp.p1_winperc_level

    winpercdiff_tourney = dp.p0_winperc_tourney - dp.p1_winperc_tourney
    winpercdiff_matchup = dp.p0_winperc_matchup - dp.p1_winperc_matchup

    rankgain_diff = dp.p0_rank_gain - dp.p1_rank_gain
    rankpointsgain_diff = dp.p0_rank_points_gain - dp.p1_rank_points_gain
    momentum_diff = dp.p0_momentum - dp.p1_momentum
...
def get_det_features(dp):  
    serveperc_diff = dp.p0_servegames_perc - dp.p1_servegames_perc 
    returnperc_diff = dp.p0_returngames_perc - dp.p1_returngames_perc 
    serve1perc_diff = dp.p0_won_1stserve_perc - dp.p1_won_1stserve_perc
    return1perc_diff = dp.p0_won_1streturn_perc - dp.p1_won_1streturn_perc
    serve2perc_diff = dp.p0_won_2ndserve_perc - dp.p1_won_2ndserve_perc
    bpsavedperc_diff = dp.p0_bp_saved_perc - dp.p1_bp_saved_perc
    return2perc_diff = dp.p0_won_2ndreturn_perc - dp.p1_won_2ndreturn_perc
    matchdur_diff = dp.p0_match_duration - dp.p1_match_duration

    # comparing how strong player1's attacking game is vs player2's defending game and viceversa
    servereturn_diff = (dp.p0_servegames_perc - dp.p1_returngames_perc) - (dp.p1_servegames_perc - dp.p0_returngames_perc)
    servereturn1_diff = (dp.p0_won_1stserve_perc - dp.p1_won_1streturn_perc) - (dp.p1_won_1stserve_perc - dp.p0_won_1streturn_perc)
    servereturn2_diff = (dp.p0_won_2ndserve_perc - dp.p1_won_2ndreturn_perc) - (dp.p1_won_2ndserve_perc - dp.p0_won_2ndreturn_perc)
...
def get_cat_features(dp):
        hand_matchup = dp.p0_hand + dp.p1_hand
        wildcard_matchup = dp.p0_wildcard + dp.p1_wildcard 
...
def create_csv_symmetric(df, csv_name='symmetric'):
    ...
    columns = basic_cols + hist_cols + det_cols + cat_cols + ['winner']
    ...
    train_csv = f'{settings.path_feature_files}/{csv_name}.csv'
    test_csv = f'{settings.path_feature_files}/{csv_name}_test.csv'

    create_train_test_csv(df, train_csv, test_csv)


def create_csv_double(df):
    features_to_drop = ['name', 'odds']
    cols_to_drop = ['p0_' + col for col in features_to_drop]
    cols_to_drop += ['p1_' + col for col in features_to_drop]
    cols_to_drop += ['tourney_date']

    cat_features = ['hand', 'wildcard']
    cat_cols = ['p0_' + col for col in cat_features]
